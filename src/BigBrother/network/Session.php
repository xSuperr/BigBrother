...
use BigBrother\network\packet\outbound\LoginDisconnectPacket;
use BigBrother\utils\InfoManager;
use BigBrother\utils\JSON;
...
class Session
{
    ...
    protected int $port;
    protected EncryptionContext $aes;
    ...
    public function process(): void
    {
        ...
        if ($this->threshold !== null) {
            $dataLength = JavaBinaryStream::readjavaVarInt($buffer, $offset);
            if ($dataLength !== 0) {
                if ($dataLength < $this->threshold) {
        ...
        if ($this->status === 2) { //Login
            ...
            $pid = JavaBinaryStream::readJavaVarInt($buffer, $offset);
            if ($pid === 0x00) {
                $sample = [];
                ...
                $data = [
                    ...
                    "description" => json_decode(JSON::toJSONInternal($this->manager->description))
                ];
            ...
            $pid = JavaBinaryStream::readJavaVarInt($buffer, $offset);
            if ($pid === 0x00) {
                $protocol = JavaBinaryStream::readJavaVarInt($buffer, $offset);
                $len = JavaBinaryStream::readJavaVarInt($buffer, $offset);
                //host name
                $offset += $len;
                //server port
                $offset += 2;
                $nextState = JavaBinaryStream::readJavaVarInt($buffer, $offset);

                if ($nextState === 1) {
                    $this->status = 1;
                } elseif ($nextState === 2) {
                    $this->status = -1;
                    // TODO: Java Multi Version??
    ...
    public function writePacket(JavaPacket $packet): void
    {
