...
class InfoManager
{
        ...
        if (is_string($packet = $this->thread->readMainToThreadPacket())) {
            ...
            switch ($pid) {
                case self::PACKET_SEND_PACKET:
                    $id = JavaBinaryStream::readInt(substr($buffer, 0, 4));
                ...
                case self::PACKET_SET_COMPRESSION:
                    $id = JavaBinaryStream::readInt(substr($buffer, 0, 4));
                    $threshold = JavaBinaryStream::readInt(substr($buffer, 4, 4));
    ...
    {
        $this->thread->pushThreadToMainPacket(chr(self::PACKET_CLOSE_SESSION) . JavaBinaryStream::writeInt($id));
    }
    ...
    {
        $this->thread->pushThreadToMainPacket(chr(self::PACKET_SEND_PACKET) . JavaBinaryStream::writeInt($id) . $buffer);
    }
    ...
    {
        $data = chr(self::PACKET_OPEN_SESSION) . JavaBinaryStream::writeInt($session->getID()) . chr(strlen($session->getAddress())) . $session->getAddress() . JavaBinaryStream::writeShort($session->getPort());
